# NanoGPT 20M FineWeb (100-step, DisTrO, norm tier gain) smoke-test config

Same as `config/test-nanogpt-20m-fineweb-100-distro`, but uses a checkpoint config that enables
...
cargo run -p psyche-centralized-local-testnet -- start \
  ...
  --num-clients 2 \
  --config-path config/test-nanogpt-20m-fineweb-100-distro-normgain \
