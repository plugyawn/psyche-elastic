...
SAME_BATCH_CALIBRATION_EVERY_STEPS="${SAME_BATCH_CALIBRATION_EVERY_STEPS:-0}"
SAME_BATCH_CALIBRATION_START_STEP="${SAME_BATCH_CALIBRATION_START_STEP:-1}"
SAME_BATCH_CALIBRATION_NO_APPLY="${SAME_BATCH_CALIBRATION_NO_APPLY:-true}"
MATFORMER_PROX_MU="${MATFORMER_PROX_MU:-0.0}"
MATFORMER_PROX_PREFIX_ONLY="${MATFORMER_PROX_PREFIX_ONLY:-true}"
DISTRO_ERROR_FEEDBACK="${DISTRO_ERROR_FEEDBACK:-false}"
DISTRO_EF_DECAY="${DISTRO_EF_DECAY:-1.0}"
...
echo "[inner] same_batch_calibration_every_steps=${SAME_BATCH_CALIBRATION_EVERY_STEPS} same_batch_calibration_start_step=${SAME_BATCH_CALIBRATION_START_STEP} same_batch_calibration_no_apply=${SAME_BATCH_CALIBRATION_NO_APPLY}"
echo "[inner] matformer_prox_mu=${MATFORMER_PROX_MU} matformer_prox_prefix_only=${MATFORMER_PROX_PREFIX_ONLY} distro_error_feedback=${DISTRO_ERROR_FEEDBACK} distro_ef_decay=${DISTRO_EF_DECAY}"
...
for tier in 0 1; do
  ...
  SAME_BATCH_CALIBRATION_NO_APPLY="${SAME_BATCH_CALIBRATION_NO_APPLY}" \
  MATFORMER_PROX_PREFIX_ONLY="${MATFORMER_PROX_PREFIX_ONLY}" \
  DISTRO_ERROR_FEEDBACK="${DISTRO_ERROR_FEEDBACK}" \
    "${CARGO_TARGET_DIR}/release/psyche-centralized-client" train \
      ...
      --same-batch-calibration-every-steps "${SAME_BATCH_CALIBRATION_EVERY_STEPS}" \
      --same-batch-calibration-start-step "${SAME_BATCH_CALIBRATION_START_STEP}" \
      --matformer-helper-fraction 0 \
      --matformer-helper-rotation-interval 16 \
      --matformer-prox-mu "${MATFORMER_PROX_MU}" \
      --bind-p2p-port "${p2p_port}" \
      --iroh-discovery "${IROH_DISCOVERY}" \
      --iroh-relay "${IROH_RELAY}" \
      --distro-apply-mode sign \
      --distro-value-mode raw \
      --distro-ef-decay "${DISTRO_EF_DECAY}" \
