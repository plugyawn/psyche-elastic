...
use psyche_modeling::{
    ...
    DistroSignErrorFeedbackConfig, DistroTierProxConfig, DistroValueMode, DummyModel,
    LlamaConfig, LlamaForCausalLM, LocalTrainer, ModelConfig, ModelLoadError, NanoGPTConfig,
    NanoGPTForCausalLM, ParallelModels, PretrainedSource, Trainer,
};
...
pub struct RunInitConfig<T: NodeIdentity, A: AuthenticatableIdentity> {
    ...
    /// If true, calibration steps fetch/compute as usual but skip distributed apply.
    ...
    /// Apply-side sign error-feedback config.
    pub distro_sign_error_feedback_config: DistroSignErrorFeedbackConfig,

    /// Tier-conditioned proximal scaling for peer updates.
    pub distro_tier_prox_config: DistroTierProxConfig,
}
...
impl<T: NodeIdentity, A: AuthenticatableIdentity + 'static> RunInitConfigAndIO<T, A> {
    ...
    pub async fn init_run(
        ...
        if !init_config.distro_sign_error_feedback_config.decay.is_finite()
            || !(0.0..=1.0).contains(&init_config.distro_sign_error_feedback_config.decay)
        {
            return Err(InitRunError::InvalidDistroConfig {
                reason: format!(
                    "distro_ef_decay must be finite and in [0,1] (got {})",
                    init_config.distro_sign_error_feedback_config.decay
                ),
            });
        }
        if !init_config.distro_tier_prox_config.mu.is_finite()
            || init_config.distro_tier_prox_config.mu < 0.0
        {
            return Err(InitRunError::InvalidDistroConfig {
                reason: format!(
                    "matformer_prox_mu must be finite and >= 0 (got {})",
                    init_config.distro_tier_prox_config.mu
                ),
            });
        }
        ...
        let data_fetcher = DataFetcher::<T, A>::new(
            ...
            init_config.same_batch_calibration_every_steps,
            init_config.same_batch_calibration_start_step,
        );
                ...
                tp_models
                    ...
                    .map(|(dp, models)| {
                        ...
                        LocalTrainer::new(
                            ...
                            init_config.distro_sign_error_feedback_config,
                            init_config.distro_tier_prox_config,
        ...
        let training = TrainingStepMetadata {
            ...
            same_batch_calibration_every_steps: init_config.same_batch_calibration_every_steps,
            same_batch_calibration_start_step: init_config.same_batch_calibration_start_step,
            same_batch_calibration_no_apply: init_config.same_batch_calibration_no_apply,
        };
