use crate::{
    ...
    optimizer::{
        ...
        DistroTierProxConfig, DistroValueMode,
    },
};
...
fn tier_prox_scale_for_peer(
    peer_tier: u8,
    parameter_name: &str,
    cfg: DistroTierProxConfig,
) -> f64 {
    let mu = cfg.mu.max(0.0);
    if mu <= 0.0 || peer_tier == 0 {
        return 1.0;
    }
    if cfg.prefix_only && matformer_prefix_dim(parameter_name).is_none() {
        return 1.0;
    }
    let scale = 1.0 / (1.0 + mu);
    if scale.is_finite() && scale > 0.0 {
        scale
    } else {
        1.0
...
struct State {
    ...
    sign_error_feedback: Box<dyn Variable>,
}
...
pub struct Distro {
    ...
    sign_error_feedback_config: DistroSignErrorFeedbackConfig,
    tier_prox_config: DistroTierProxConfig,
    state: Vec<State>,
...
impl Distro {
    pub fn new(
        ...
        for variable in vs.variables() {
            state.push(State {
                ...
                sign_error_feedback: variable
                    .zeros_like(format!("{}.sign_error_feedback", variable.name())),
            });
    ...
    pub fn apply(&mut self, vars: &dyn CausalLM, results: &[Vec<DistroResult>], lr: f64) {
        ...
        let mut telemetry_sign_ef_residual_l2_sq: f64 = 0.0;
        let mut telemetry_sign_ef_residual_linf: f64 = 0.0;
        let mut telemetry_sign_ef_residual_count: u64 = 0;
        ...
        let mut telemetry_tier_prox_scale_sum: f64 = 0.0;
        let mut telemetry_tier_prox_scale_min: f64 = f64::INFINITY;
        let mut telemetry_tier_prox_scale_max: f64 = 0.0;
        let mut telemetry_tier_prox_scale_count: u64 = 0;
        ...
        for (index, var) in vars.variables().enumerate() {
            ...
            if same_shape {
                ...
                if apply_mixer_for_param {
                    ...
                    for (peer_results, sparse_val) in results.iter().zip(values.iter()) {
                        ...
                        let peer_tier = res
                            .peer_metadata
                            .map(|m| m.matformer_tier)
                            .unwrap_or(0);
                        let prox_scale = tier_prox_scale_for_peer(
                            peer_tier,
                            var.name(),
                            self.tier_prox_config,
                        );
                        if (prox_scale - 1.0).abs() > f64::EPSILON {
                            let _ = aligned.g_mul_scalar_(prox_scale);
                        }
                        telemetry_tier_prox_scale_sum += prox_scale;
                        telemetry_tier_prox_scale_min =
                            telemetry_tier_prox_scale_min.min(prox_scale);
                        telemetry_tier_prox_scale_max =
                            telemetry_tier_prox_scale_max.max(prox_scale);
                        telemetry_tier_prox_scale_count += 1;
                        let n_sq = aligned
                            .to_kind(Kind::Float)
                            .pow_tensor_scalar(2)
                            .sum(Kind::Float)
                    ...
                    if !results.is_empty() {
                        let prox_mean = results
                            .iter()
                            .map(|peer_results| {
                                let tier = peer_results[index]
                                    .peer_metadata
                                    .map(|m| m.matformer_tier)
                                    .unwrap_or(0);
                                tier_prox_scale_for_peer(
                                    tier,
                                    var.name(),
                                    self.tier_prox_config,
                                )
                            ...
                            .sum::<f64>()
                            / results.len() as f64;
                        if (prox_mean - 1.0).abs() > f64::EPSILON {
                            let _ = aligned.g_mul_scalar_(prox_mean);
                        }
                        telemetry_tier_prox_scale_sum += prox_mean;
                        telemetry_tier_prox_scale_min =
                            telemetry_tier_prox_scale_min.min(prox_mean);
                        telemetry_tier_prox_scale_max =
                            telemetry_tier_prox_scale_max.max(prox_mean);
                        telemetry_tier_prox_scale_count += 1;
                    }
                    if want_sign_flip_stats {
            ...
            match self.apply_mode {
                DistroApplyMode::Sign => {
                    if self.sign_error_feedback_config.enabled {
                        let decay = self.sign_error_feedback_config.decay.clamp(0.0, 1.0);
                        let state = self.state.get_mut(index).expect("state len matches vars");
                        let mut residual = state.sign_error_feedback.logical_tensor();
                        if (decay - 1.0).abs() > f64::EPSILON {
                            let _ = residual.g_mul_scalar_(decay);
                        }
                        let compensated =
                            grad.to_kind(Kind::Float) + residual.to_kind(Kind::Float);
                        let signed = compensated.sign();
                        let new_residual = &compensated - &signed;
                        residual.copy_(&new_residual.to_kind(residual.kind()));
                        grad.copy_(&signed.to_kind(grad.kind()));

                        let residual_f = residual.to_kind(Kind::Float);
                        telemetry_sign_ef_residual_l2_sq += residual_f
                            .pow_tensor_scalar(2)
                            .sum(Kind::Float)
                            .double_value(&[]);
                        telemetry_sign_ef_residual_linf = telemetry_sign_ef_residual_linf
                            .max(residual_f.abs().max().double_value(&[]));
                        telemetry_sign_ef_residual_count += 1;
                    } else {
                        let _ = grad.sign_();
                    }
                }
        ...
        if telemetry_grad_tensors > 0 {
            ...
            if should_log {
                ...
                let tier_prox_scale_mean = if telemetry_tier_prox_scale_count > 0 {
                    telemetry_tier_prox_scale_sum / telemetry_tier_prox_scale_count as f64
                } else {
                    0.0
                };
                let tier_prox_scale_min = if telemetry_tier_prox_scale_count > 0 {
                    telemetry_tier_prox_scale_min
                } else {
                    0.0
                };
                let tier_prox_scale_max = if telemetry_tier_prox_scale_count > 0 {
                    telemetry_tier_prox_scale_max
                } else {
                    0.0
                };
                let sign_ef_residual_l2 = telemetry_sign_ef_residual_l2_sq.sqrt();
                let sign_ef_residual_linf = if telemetry_sign_ef_residual_count > 0 {
                    telemetry_sign_ef_residual_linf
                } else {
                    0.0
                };
                ...
                info!(
                    ...
                    sign_error_feedback_enabled = self.sign_error_feedback_config.enabled,
                    sign_error_feedback_decay = self.sign_error_feedback_config.decay,
                    sign_ef_residual_count = telemetry_sign_ef_residual_count,
                    sign_ef_residual_l2 = sign_ef_residual_l2,
                    sign_ef_residual_linf = sign_ef_residual_linf,
                    ...
                    tier_prox_mu = self.tier_prox_config.mu,
                    tier_prox_prefix_only = self.tier_prox_config.prefix_only,
                    tier_prox_scale_count = telemetry_tier_prox_scale_count,
                    tier_prox_scale_mean = tier_prox_scale_mean,
                    tier_prox_scale_min = tier_prox_scale_min,
                    tier_prox_scale_max = tier_prox_scale_max,
                    lr = lr,
